...
describe("ProfileService", () => {
  ...
  describe("createCustomerProfile", () => {
    const profileData = { currentArea: "Seoul", moveType: [MoveType.HOME] };
    ...
    test("고객 프로필 생성 성공 시, 프로필 정보와 토큰을 반환해야 합니다.", async () => {
    ...
    test("사용자 타입이 고객이 아닌 경우 CustomError를 던져야 합니다.", async () => {
    ...
    test("프로필이 이미 존재하는 경우 CustomError를 던져야 합니다.", async () => {
      mockedAuthRepository.findById.mockResolvedValue(authUser as any);
  ...
  describe("updateCustomerProfile", () => {
    ...
    test("프로필과 사용자 정보를 성공적으로 수정해야 합니다.", async () => {
    ...
    test("비밀번호 변경을 성공적으로 처리해야 합니다.", async () => {
      const passwordData = {
      ...
      mockedAuthRepository.findById.mockResolvedValueOnce(authUserWithPassword as any);
      (mockedBcrypt.compare as jest.Mock).mockResolvedValue(true);
      (mockedBcrypt.hash as jest.Mock).mockResolvedValue("hashedNewPassword");
      mockedAuthRepository.updateAuthUser.mockResolvedValue(undefined);

      mockedProfileRepository.updateCustomerProfile.mockResolvedValue(updatedProfile as any);
      mockedAuthRepository.findById.mockResolvedValueOnce(updatedAuthUser as any);
    ...
    test("현재 비밀번호가 틀리면 CustomError를 던져야 합니다.", async () => {
      const passwordData = {
    ...
    test("수정할 프로필 정보가 없으면 CustomError를 던져야 합니다.", async () => {
  ...
  describe("createDriverProfile", () => {
    ...
    test("기사 프로필 생성 성공 시, 프로필 정보와 토큰을 반환해야 합니다.", async () => {
      mockedAuthRepository.findById.mockResolvedValue(authUser as any);
    ...
    test("사용자 타입이 기사가 아닌 경우 CustomError를 던져야 합니다.", async () => {
  ...
  describe("updateDriverProfile", () => {
    ...
    test("기사 프로필 정보를 성공적으로 수정해야 합니다.", async () => {
    ...
    test("서비스 지역 정보를 성공적으로 수정해야 합니다.", async () => {
      mockedProfileRepository.findDriverByAuthUserId.mockResolvedValue({ id: driverId } as any);
    ...
    test("기사 프로필을 찾을 수 없는 경우 CustomError를 던져야 합니다.", async () => {
      mockedProfileRepository.findDriverByAuthUserId.mockResolvedValue(null);
  ...
  describe("updateDriverBasicProfile", () => {
    const updateData = { name: "기사새이름" };
    const updatedAuthUser = { id: authUserId, name: "기사새이름" };

    test("기사 기본 정보를 성공적으로 수정해야 합니다.", async () => {
      mockedAuthRepository.updateAuthUser.mockResolvedValue(undefined);
      mockedAuthRepository.findById.mockResolvedValue(updatedAuthUser as any);

      const result = await profileService.updateDriverBasicProfile(authUserId, updateData);

      expect(mockedAuthRepository.updateAuthUser).toHaveBeenCalledWith(authUserId, { name: "기사새이름" });
      expect(result).toEqual({ authUser: updatedAuthUser });
    });
