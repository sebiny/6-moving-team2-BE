...
export const getPendingEstimates = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!customerId) throw new CustomError(401, "고객 ID가 누락되었습니다.");

  const estimates = await customerEstimateService.getPendingEstimates(customerId);
...
export const getReceivedEstimates = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    const customerId = req.user?.customerId;
    if (!customerId) throw new CustomError(401, "인증된 사용자가 아닙니다.");

    const estimates = await customerEstimateService.getReceivedEstimates(customerId);
    res.status(200).json(estimates);
  } catch (error) {
    next(error);
  }
